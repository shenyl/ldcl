...
QDlgResultCl::QDlgResultCl( QString strTabName, QWidget* parent, Qt::WindowFlags flags )
    : QDialog(parent, flags)
{
    this->strTabName = strTabName ;

//    tabModel = new QSqlTableModel ;
    tabModel = new QSqlTableModelResult ;
...
//    bottomLayout -> addWidget(buttonDelete);
    ...
    setTitle( );
    tabView->hideColumn( 4 ); //隐藏指定的列 备注列
    slotQuery( );

}

QDlgResultCl::~QDlgResultCl()
{

}

void QDlgResultCl::retrieveTableView()
{
    tabModel->select( );
}

void QDlgResultCl::insertRow()
{
...
//格式化时间串
QString QDlgResultCl::formatTime( QString strTime )
{
...
void QDlgResultCl::slotQuery( )
{
    QString strFilter ;

    strFilter = QString("dt > '%1' and dt < '%2' ")
            .arg( formatTime( edtStart->text()) ).arg( formatTime(edtEnd->text()) );

    qDebug( ) << strFilter ;
    tabModel->setFilter( strFilter );
    tabModel->select( );
}

//存成xls文件
void QDlgResultCl::slotSaveXls( )
{
    QFunc func ;
    func.saveXLS( tabView, tabModel );
}

void QDlgResultCl::setTitle( )
{
    QSqlQuery query;
    QString strSql ;
    int i = 0 ;
